...
D20SeccEngine::D20SeccEngine(io::StreamOutputView output_view, session::SessionConfig config,
                             ...
                             std::optional<io::sha512_hash_t> vehicle_cert_hash, bool skip_app_protocol_negotiation) :
    message_exchange(output_view),
    ctx(std::move(callbacks), std::move(config), pause_ctx, active_control_event, message_exchange, timeouts),
    // SessionSetup only emits the ev_information feedback when the SupportedAppProtocol handshake ran
    // in-session; with an external SAP the application already has that information.
    fsm(ctx.create_state<d20::state::SessionSetup>(skip_app_protocol_negotiation)) {
...
void D20SeccEngine::on_control_event(const d20::ControlEvent& event) {
    ...
    } else if (const auto control_data = ctx.get_control_event<d20::UpdatePowersupplyLimits>()) {
        // The power-supply capabilities feed the DC_ChargeParameterDiscoveryRes offer, which a
